 ...
 */
export class IpcTrafficFileWriter {
  ...
  relocate(newPath: string): void {
    if (newPath === this._filePath) return;
    try {
      ...
      if (this._headerWritten) {
        ...
        try { fs.unlinkSync(this._filePath); } catch { /* ignore */ }
      }
  ...
  write(record: IpcTrafficRecord): void {
    ...
    const row =
      `${_csvCell(ts)},${_csvCell(record.direction)},${_csvCell(record.kind)},` +
...
function _formatTs(epochSeconds: number): string {
